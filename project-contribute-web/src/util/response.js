...
export const Bad_Request=400; // 参数错误
/* 判断调用是否成功*/
export function isSuccess(res) {
	return res.data.status.code == Success
}
/* 判断是否没有权限调用 */
export function isForbidden(res) {
	return res.data.status.code == Forbidden
}
/* 判断是否未登录 */
export function isUnauthorized(res) {
	return res.data.status.code == Unauthorized
}
/* 判断是否服务器发生异常 */
export function isError(res) {
	return res.data.status.code == Server_Error
}
/* 判断是否参数存在问题异常 */
export function isBadRequest(res) {
	return res.data.status.code == Bad_Request
}
/*
方法名：获取数据集
参数：res，http请求返回数据
